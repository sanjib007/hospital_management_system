...
class Demo_auth_model extends CI_Model {
	...
	function register_account()
	{
		...
		if ($this->form_validation->run())
		{
			...
			if($this->input->post('visit') && $this->input->post('vtime')){
				$profile_data['default_visit_price'] = $this->input->post('visit');
				$profile_data['default_visit_duration'] = $this->input->post('vtime');
                $profile_data['previous_visit_price'] = $this->input->post('previous_vist_price');
                $profile_data['previous_visit_day'] = $this->input->post('previous_vist_time');
                $profile_data['doctor_prescribe_info']= $this->input->post('editor1');
			}
			...
			$response = $this->flexi_auth->insert_user($email, $username, $password, $profile_data, $this->input->post('type'), $instant_activate);
